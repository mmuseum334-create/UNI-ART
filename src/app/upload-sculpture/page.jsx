 ...
 * Formulario multi-paso para información de escultura y carga de 20-50 fotos
 * PROTEGIDO: Requiere permisos de creación de esculturas
 */
'use client'
...
const UploadSculptureContent = () => {
  const { user, isAuthenticated } = useAuth();
  const { canCreate, RESOURCES: RES } = usePermissions();
  ...
  }, [images, MAX_FILE_SIZE, MAX_IMAGES]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setErrors({
          submit: response.error || 'Error al subir la escultura. Intenta nuevamente.',
        });
      }
    } catch (error) {
    ...
    return (
      <div className="space-y-4">
        ...
        >
          ...
          <h3 className="text-lg font-medium text-slate-900 mb-2 dark:text-white">
...
export default function UploadSculpture() {
  return (
    <ProtectedRoute
      requireAuth
